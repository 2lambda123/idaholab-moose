...
sol_data = pd.read_csv('junction_4pipes_closed_out.csv')
sol_nocorr = pd.read_csv('junction_4pipes_closed_nocorr.csv')

def makePlot(pipe_loc_str):
  exp_file_name = 'data/4pipes_closed_' + pipe_loc_str + '.csv'
  exp_data = pd.read_csv(exp_file_name)
  exp_data = exp_data.sort_values(by=['time'])

  pp_name = 'p_' + pipe_loc_str
  plot_name = '4pipes_closed_' + pipe_loc_str + '.png'
  ...
  plt.plot(sol_nocorr['time'], sol_nocorr[pp_name] / 1e3 - p_low, linestyle='--', marker='', color='indianred', label='THM, no corr.')
