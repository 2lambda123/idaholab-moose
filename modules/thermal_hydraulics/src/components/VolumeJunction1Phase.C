...
VolumeJunction1Phase::validParams()
{
  ...
  params.addParam<bool>(
      "use_scalar_variables", false, "True if the junction variables are scalar variables");
...
VolumeJunction1Phase::setupMesh()
{
  ...
  if (!_use_scalar_variables)
  {
    ...
    _junction_elem_id = elem->id();
    const auto subdomain_id = mesh().getNextSubdomainId();
    ...
    for (unsigned int i = 0; i < elem_ids.size(); i++)
      getTHMProblem().augmentSparsity(elem_ids[i], _junction_elem_id);
  }
}
...
VolumeJunction1Phase::buildVolumeJunctionUserObject()
{
...

  {
    ...
    if (!_use_scalar_variables)
      params.set<dof_id_type>("junction_elem_id") = _junction_elem_id;
