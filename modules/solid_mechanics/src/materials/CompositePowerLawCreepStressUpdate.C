...
CompositePowerLawCreepStressUpdateTempl<is_ad>::validParams()
{
  ...
  params.addRequiredParam<std::vector<Real>>(
      "coefficient", "Leading coefficient / Dorn Constant in power-law equation.");
  ...
  params.addRequiredParam<std::vector<Real>>(
      "activation_energy", "Activation energy for Arrhenius-type equation of the Dorn Constant");
  params.addParam<Real>("gas_constant", 8.3143, "Universal gas constant");
  params.addParam<Real>("start_time", 0.0, "Start time (if not zero)");
  params.addRequiredParam<std::vector<std::string>>(
      "switching_functions", "List of switching functions for each material");
  return params;
}
...
CompositePowerLawCreepStressUpdateTempl<is_ad>::CompositePowerLawCreepStressUpdateTempl(
  ...
  : RadialReturnCreepStressUpdateBaseTempl<is_ad>(parameters),
    ...
    _switchingFunc(this->template getParam<std::vector<std::string>>("switching_functions")),
    _num_materials(_switchingFunc.size())
